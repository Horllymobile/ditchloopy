...
object GeminiService {
    ...
    /**
     * Generates a batch of personalized DitchLoopy Invitations using Gemini.
     */
    @Suppress("UNCHECKED_CAST")
    suspend fun generatePersonalizedInvitationsBatch(
        count: Int,
        mood: String,
    ...
    ): List<Invitation> = withContext(Dispatchers.IO) {
        if (!isApiKeyAvailable()) {
            Log.w("GeminiService", "Gemini API key is not available. Using local curation.")
            return@withContext emptyList()
        }

        val completedText = if (completedTitles.isNotEmpty()) {
            """
            === MEMORY BUFFER: COMPLETED JOURNEY HISTORY ===
            The user has successfully completed the following experiences in their past loop iterations.
            You MUST review this history carefully before formulating new invitations:
            ${completedTitles.joinToString("\n") { "- $it" }}
            
            CRITICAL DIRECTIVES:
            1. STRICT UNIQUENESS: Do NOT repeat, rephrase, or adapt any of the titles, core activities, or specific instructions from the completed journey history above. Every new recommendation must be an entirely novel concept.
            2. CONTEXTUAL EVOLUTION: Design the new invitations as a progressive continuation of their journey. Seek to complement, expand, or gently contrast with their completed history, matching the requested vibe while exploring new realms of presence.
            ================================================
            """.trimIndent()
        } else {
            """
            === MEMORY BUFFER: COMPLETED JOURNEY HISTORY ===
            The user has no completed experiences recorded in their journey history yet.
            This is their initial embarkation. Provide introductory but highly evocative and distinct mindful experiences.
            ================================================
            """.trimIndent()
        }
        ...
        val prompt = """
            ...
            We need you to generate exactly $count highly evocative, creative, and completely unique mindful experiences (called "Invitations") tailored to the user's current states:
            - Mood/Vibe: $mood
            ...
            Each experience should not feel like a chore or habit. It must focus on sensory observation, memory creation, or artistic presence. Keep them safe, creative, and serene.

            Format the response strictly as a single JSON object with a field "invitations" containing the list of invitations, and do NOT wrap it in markdown code blocks:
            {
              "invitations": [
                {
        ...
        try {
            client.newCall(request).execute().use { response ->
                ...
                val rawText = geminiResp?.candidates?.firstOrNull()?.content?.parts?.firstOrNull()?.text ?: return@withContext emptyList()
                ...
                // Parse the inner generated JSON list
                val moshiInner = Moshi.Builder().addLast(KotlinJsonAdapterFactory()).build()
                val mapAdapter = moshiInner.adapter(Map::class.java)
                val map = mapAdapter.fromJson(jsonString) as? Map<String, Any> ?: return@withContext emptyList()
                
                val rawList = map["invitations"] as? List<Map<String, Any>> ?: return@withContext emptyList()
                
                rawList.mapIndexed { idx, itemMap ->
                    val title = (itemMap["title"] as? String) ?: "An Unplanned Moment"
                    val description = (itemMap["description"] as? String) ?: "Step away from your screen for a brief period of quiet awareness."
                    val category = (itemMap["category"] as? String) ?: "Discover"
                    val difficulty = (itemMap["difficulty"] as? String) ?: "Easy"
                    val duration = (itemMap["duration"] as? String) ?: "15m"
                    val cost = (itemMap["cost"] as? String) ?: "Free"

                    Invitation(
                        id = "custom_" + System.currentTimeMillis() + "_" + idx,
                        title = title,
            ...
            Log.e("GeminiService", "Error generating invitations batch: ${e.message}", e)
            emptyList()
        }
    }
